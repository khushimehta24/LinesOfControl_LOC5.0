...
export const incomingCallWrapperStyle = (props, keyframes) => {
	...
	if (props.hasOwnProperty("widgetsettings")) {
		if (
			props.widgetsettings.hasOwnProperty("dockedview") &&
			props.widgetsettings.dockedview
		) {
			//if chat window is open
			if (
				props.widgetsettings.hasOwnProperty("launched") &&
				props.widgetsettings.launched
			) {
				...
				if (
					props.widgetsettings.hasOwnProperty("alignment") &&
					props.widgetsettings.alignment === "left"
				) {
					rightPos = "unset";
					leftPos = "20px";
					...
					if (props.widgetsettings.hasOwnProperty("height")) {
						if (props.widgetsettings.height.includes("px")) {
							const widgetHeight = props.widgetsettings.height.replace(
								"px",
								""
							);
...
export const ButtonStyle = (props, action) => {
	const backgroundColor = action
		? `${props.theme.backgroundColor.blue}!important`
		: `${props.theme.backgroundColor.red}!important`;
